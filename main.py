...
async def cancel_handler(message: Message, state: FSMContext) -> None:
    ...
    if current_state is None:
        await message.answer("Нет активных действий для отмены.", reply_markup=main_menu_keyboard)
        return
    await state.clear()
...
async def command_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Добро пожаловать в медиапространство CYBERQELN", reply_markup=main_menu_keyboard)
...
async def get_users_base(message: Message) -> None:
    ...
    for i, (tg_id, user_data) in enumerate(users.items(), 1):
        tg_id = int(tg_id)
        status = ""
        if user_data['is_banned']: status += " 🚫 BANNED"
        if user_data['is_admin']: status += " 👮 ADMIN"
        if tg_id in ADMIN_IDS: status += " 👑 SUPER ADMIN"

        user_link = f"@{user_data['username']}" if user_data['username'] != "N/A" else "Нет юзернейма"
        
        response_parts.append(
            f"🔹 <b>{i}. {user_data['full_name']}</b>{status}\n"
            f"   🆔 ID: <code>{tg_id}</code>\n"
            f"   👤 Link: {user_link}\n"
            f"   📅 Дата: {user_data['join_date'].split(' ')[0]}"
        )
...
async def process_media(message: Message, state: FSMContext) -> None:
    ...
    caption = (
        f"Новая заявка!\n\n"
        f"👤 Никнейм в игре: {data['nickname']}\n"
    ...
    try:
        ...
        await message.answer("Спасибо! Твои данные отправлены.")
